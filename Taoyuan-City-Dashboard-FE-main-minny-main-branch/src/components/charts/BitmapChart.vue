...
const colorScale = computed(() => {
	const ranges = props.chart_config.color.map((el, index) => ({
		to: Math.floor(
			(BitmapData.value.highest / props.chart_config.color.length) *
	...
	tooltip: {
		custom: function ({ series, seriesIndex, dataPointIndex, w }) {
			...
			return (
				...
				"<h6>" +
				`${w.globals.seriesNames[seriesIndex]}-${w.config.series[seriesIndex].data[dataPointIndex][0]}` +
				"</h6>" +
				"<span>"+
...
<template>
	<div v-if="activeChart === 'BitmapChart'" class="BitmapChart">
		<apexchart
			width="100%"
			height="250px"
			type="scatter"
